...
const DecisionMarketsFactoryFull = artifacts.require('DecisionMarketsFactory.sol')
const DecisionMarketsMock = artifacts.require('DecisionMarketsMock.sol')
const DecisionMarketsFactoryMock = artifacts.require('DecisionMarketsFactoryMock.sol')
...
contract('Futarchy', (accounts) => {
  ...
  let futarchyBase
  let futarchy, token, priceOracleFactory, decisionMarketsFactory, lmsrMarketMaker
  let executionTarget
  ...
  before(async () => {
    ...
    decisionMarketsFactoryFull = await deployFutarchyMasterCopies()
    decisionMarketsFactoryMock = await DecisionMarketsFactoryMock.new()
  })
  ...
  describe('initialize()', async () => {
    ...
    it('sets decisionMarketsFactory', async () => {
      expect(await futarchy.decisionMarketsFactory()).to.equal(decisionMarketsFactoryMock.address)
    })
  ...
  describe('transitionDecision()', () => {
    ...
    beforeEach(async () => {
      ...
      decisionMarkets = DecisionMarketsMock.at((await futarchy.decisions(0))[0])
    })

    describe('when categoricalEvent is not yet resolved', () => {
      it('resolves the categoricalEvent', async () => {
        expect(await Event.at(await decisionMarkets.getCategoricalEvent()).isOutcomeSet()).to.equal(false)
        await timeTravel(TRADING_PERIOD + 1)
        await futarchy.transitionDecision(0)
        expect(await Event.at(await decisionMarkets.getCategoricalEvent()).isOutcomeSet()).to.equal(true)
      })

      it('resolves the decisionMarkets', async () => {
        expect(await decisionMarkets.isOutcomeSet()).to.equal(false)
        await timeTravel(TRADING_PERIOD + 1)
        await futarchy.transitionDecision(0)
        expect(await decisionMarkets.isOutcomeSet()).to.equal(true)
      })
      ...
      it('sets passed to true if decision passed', async () => {
        await decisionMarkets.mock_setWinningMarketIndex(0)
        expect((await futarchy.decisions(0))[7]).to.equal(false)
        await timeTravel(TRADING_PERIOD + 1)
        await futarchy.transitionDecision(0)
      ...
      it('sets passed to false if decision failed', async () => {
        await decisionMarkets.mock_setWinningMarketIndex(1)
        expect((await futarchy.decisions(0))[7]).to.equal(false)
        await timeTravel(TRADING_PERIOD + 1)
        await futarchy.transitionDecision(0)
        expect((await futarchy.decisions(0))[7]).to.equal(false)
    ...
    describe('when categoricalEvent is already resolved', () => {
      it('sets winning decision.passed to true if it is not set already', async () => {
        await Event.at(await decisionMarkets.categoricalEvent()).setOutcome()
        await decisionMarkets.setOutcome();
        await decisionMarkets.mock_setWinningMarketIndex(0)
      ...
      it('closes markets ready to be closed', async () => {
        ...
        let winningMarket = MarketMock.at(await decisionMarkets.markets(winningMarketIndex))

        await Event.at(await decisionMarkets.categoricalEvent()).setOutcome()
        await decisionMarkets.setOutcome();
        await decisionMarkets.mock_setWinningMarketIndex(winningMarketIndex)
  ...
  describe('closeDecisionMarkets() unit tests', () => {
    let script, metadata, decisionId, decisionMarkets, keccak
    let yesMarket, noMarket, yesEvent, noEvent, yesToken, noToken, yesLongToken, yesShortToken, noLongToken, noShortToken

    beforeEach(async () => {
      // large setup not required bc no integration with full decisionMarkets here
      initializeFutarchy()
      script = 'QmWmyoMoctfbAaiEs2G46gpeUmhqFRDW6KWo64y5r581Vz'
      metadata = 'Give voting rights to all kitties in the world'
      await token.approve(futarchy.address, MARKET_FUND_AMOUNT +  (40 * 10 ** 18), {from: root})
    ...
    it('calls decisionMarkets.close()', async () => {
      expect(await decisionMarkets.mock_closed()).to.equal(false)
      await futarchy.closeDecisionMarkets(0)
      expect(await decisionMarkets.mock_closed()).to.equal(true)
    })
    ...
    it('transfers the appropriate refund to creator if refund amount is less than the full market funding', async () => {
      ...
      await decisionMarkets.mock_setRefundAmount(lowRefundAmount)
  ...
  describe('executeDecision()', async () => {
    let script, metadata, decisionId, currentBlockNumber, decisionMarkets, exeutionTarget
  ...
  describe('buyMarketPositions()', async () => {
    let script, metadata, decisionMarkets
    let yesLongToken, yesShortToken, noLongToken, noShortToken

    beforeEach(async () => {
      initializeFutarchy({_decisionMarketsFactoryAddr: decisionMarketsFactoryFull.address})
      script = 'QmWmyoMoctfbAaiEs2G46gpeUmhqFRDW6KWo64y5r581Vz'
      metadata = 'Give voting rights to all kitties in the world'
      await token.approve(futarchy.address, MARKET_FUND_AMOUNT +  (40 * 10 ** 18), {from: root})
      await futarchy.newDecision(script, metadata, LOWER_BOUND, UPPER_BOUND)
      decisionMarkets = FutarchyDecisionMarketsFull.at((await futarchy.decisions(0))[0])
    })
    ...
    describe('when trader is trading again on the same market', async () => {
      beforeEach(async () => {
        let yesMarket = StandardMarketWithPriceLogger.at(await decisionMarkets.markets(0))
    ...
    describe('for noMarket trades', async () => {
      beforeEach(async () => {
        let noMarket = StandardMarketWithPriceLogger.at(await decisionMarkets.markets(1))
  ...
  describe('sellMarketPositions()', async () => {
    ...
    beforeEach(async () => {
      ...
      decisionMarkets = FutarchyDecisionMarketsFull.at((await futarchy.decisions(0))[0])
      yesMarket = StandardMarketWithPriceLogger.at(await decisionMarkets.markets(0))
      noMarket = StandardMarketWithPriceLogger.at(await decisionMarkets.markets(1))
    ...
    describe('when scalar prediction markets are closed', () => {
      it('reverts', async () => {
        ...
        await decisionMarkets.setOutcome()
        await Event.at(await decisionMarkets.categoricalEvent()).setOutcome()
        const oracle = CentralizedTimedOracle.at(await Event.at(await StandardMarket.at(await decisionMarkets.markets(0)).eventContract()).oracle())
  ...
  describe('redeemWinningCollateralTokens()', () => {
    ...
    describe('when decisionMarkets is not yet set', () => {
      it('reverts', async () => {
        await futarchy.buyMarketPositions(0, TWENTY, [0, TWO], [FIVE,0])
        await timeTravel(TRADING_PERIOD + 1)
        return assertRevert(async () => {
  ...
  describe('redeemWinnings()', () => {
    beforeEach(async () => {
      ...
      await futarchy.newDecision(script, metadata, LOWER_BOUND, UPPER_BOUND)
      decisionMarkets = FutarchyDecisionMarketsFull.at((await futarchy.decisions(0))[0])
      yesMarketAddr = await decisionMarkets.markets(0)
      noMarketAddr = await decisionMarkets.markets(1)
  ...
  async function initializeFutarchy(customParams = {}) {
    const {
      ...
      _decisionMarketsFactoryAddr = decisionMarketsFactoryMock.address,
  ...
  async function setScalarEvent(futarchy, decisionId, resolvedPrice) {
    let decisionMarkets = FutarchyDecisionMarketsFull.at((await futarchy.decisions(decisionId))[0])
    let winningIndex = (await decisionMarkets.winningMarketIndex()).toNumber()
    const oracle = CentralizedTimedOracle.at(await Event.at(await StandardMarket.at(await decisionMarkets.markets(0)).eventContract()).oracle())
    await oracle.setOutcome(resolvedPrice)
    await Event.at(await StandardMarket.at(await decisionMarkets.markets(winningIndex)).eventContract()).setOutcome()
  }
...
async function logTokenBalances(futarchy, addrs = []) {
  let decisionMarkets = FutarchyDecisionMarketsFull.at((await futarchy.decisions(0))[0])
  let yesMarket = StandardMarketWithPriceLogger.at(await decisionMarkets.markets(0))
  let noMarket = StandardMarketWithPriceLogger.at(await decisionMarkets.markets(1))
  let yesEvent = ScalarEvent.at(await yesMarket.eventContract())
  let noEvent = ScalarEvent.at(await noMarket.eventContract())
...
async function deployFutarchyMasterCopies() {
  ...
  const decisionMarkets = await FutarchyDecisionMarketsFull.new()
  ...
  const decisionMarketsFactory = await DecisionMarketsFactoryFull.new(decisionMarkets.address, eventFactory.address, standardMarketWithPriceLoggerFactory.address)
  return decisionMarketsFactory
}
